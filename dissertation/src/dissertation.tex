...
Fast Fourier Transforms is a family of algorithms which are very useful for the computation of Discrete Fourier Transforms which is applied in many practical scenarios for several areas.
...
In this introductory chapter we present a rudimentary introduction to the Fourier Transform in \autoref{sec:fourier-transform} and describe the discrete version of the Fourier Transform concept in \autoref{sec:discrete-fourier-transform} and finalizing with the state of the art of the most popular algorithms in \autoref{sec:fast-fourier-transform}.

\section{Fourier Transform} \label{sec:fourier-transform}

\subsection{What is the Fourier Transform} \label{subsec:what-is-the-fourier-transform}
...
The definition of \autoref{eq:fourier-transform} integral is only valid if the integral exists for every value of the parameter \(f\). 
...
If we take into account \autoref{eq:euler}, we can replace the Fourier Transform for an equivalent by resolving the Euler's constant for a sine and cosine sum.
...
The \acrfull{fft} is a family of algorithms that effectively compute the Discrete Fourier Transform (DFT) of a sequence and its inverse. These algorihtms essentially compute the the same result as the DFT but the direct usage of the DFT formulation is too slow for its applications. Thus, FFT algorithms exploit the DFT matrix structure by employing a divide-and-conquer approach \cite{chu1999inside} to segment its application.
...
% 1.1
\paragraph{}
There are many algorithms and aproaches on the FFT family such as the well known Cooley-Tukey, known for its simplicity and effectiveness to compute any sequence with size as a power of two, but also Rader's algorithm \cite{rader1968discrete} and Bluestein's algorithm \cite{bluestein1970linear} which both deal with prime sized sequences, and even the Split-radix FFT \cite{yavne1968economical} that recursively expresses a DFT of length \(N\) in terms of one smaller DFT of length \(N/2\) and two smaller DFTs of length \(N/4\). 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{img/cpu_gpu.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{tese/img/2d_fft.png}
...
We can already see in \autoref{lst:glsl-ct-vetical} the above code a lot of branching that might be undesirable on the GPU  ... % TODO: Reference that thing where a warp branching has a huge performance hit due to the way it is implemented
...
% Results:
% - List some graphs with evidence of the grade of performance difference between the implementations
% - Results obtained using nau3d engine integrated profiller
...
The cuFFT library is the NVIDIA framework designed to provide high performance \acrshort{fft} exclusively on its own GPUs that supports a wide range of \acrshort{fft} inputs and settings that compute \acrshort{fft}s effitiently on NVIDIA GPUs.
...
The \acrshort{cufft} library uses algorithms highly optimized for input sizes that can be written in the form $2^a \times 3^b \times 5^c \times 7^d$, so it factorizes the input size to allow arbitrary sized FFT sequence wile lower prime factors have have better performance.
\newline



The results of cuFFT out-of-place benchmarks can be found in \autoref{fig:cufft-vs-glsl-radix2} and \autoref{fig:cufft-vs-glsl-radix4}.

\section{Implementation analysis in GLSL}
% TODO:
% - Compare all implementations from section 5.3.2
% - What was expected for the results
% - Details reasons why the results are this way 
% - Include a comparison of the kernel size with loop unroll, and code improvements

% Results:
% NOTE: Should i include in-place data too?
% - [x] cuFFT out-of-place and GLSL Radix-2 Cooley-Tukey
% - [x] cuFFT out-of-place vs GLSL Radix-2 Stockham
% - [x] cuFFT out-of-place vs GLSL Radix-4 Stockham

\begin{figure}[h] 
    \centering
    \includegraphics[width=0.5\textwidth]{img/results/cufft_vs_glsl_radix2.png}
    \caption{2D FFT benchmarks of out-of-place cuFFT and GLSL Radix-2 algorithms}
    \label{fig:cufft-vs-glsl-radix2}
\end{figure}

\begin{figure}[h] 
    \centering
    \includegraphics[width=0.5\textwidth]{img/results/cufft_vs_glsl_radix4.png}
    \caption{2D FFT benchmarks of out-of-place cuFFT and GLSL Radix-4 Stockham algorithm}
    \label{fig:cufft-vs-glsl-radix4}
\end{figure}



\section{Implementation analysis in CUDA}
% - What was expected for the results
% Results:
% - [x] CUDA vs GLSL Radix-2 Cooley-Tukey
% - [x] CUDA vs GLSL Radix-2 Stockham
% - [x] CUDA vs GLSL Radix-4 Stockham

\begin{figure}[h] 
    \centering
    \includegraphics[width=0.5\textwidth]{img/results/cuda_vs_glsl_radix2.png}
    \caption{2D FFT benchmarks of CUDA and GLSL Radix-2 algorithms}
    \label{fig:cufft-vs-glsl-radix2}
\end{figure}

\begin{figure}[h] 
    \centering
    \includegraphics[width=0.5\textwidth]{img/results/cuda_vs_glsl_radix4.png}
    \caption{2D FFT benchmarks of CUDA and GLSL Radix-4 Stockham algorithm}
    \label{fig:cufft-vs-glsl-radix4}
\end{figure}
...
\chapter{Cooley-Tukey FFT} \label{apdx:ctfft}

\begin{lstlisting}[language=C,caption={FFT Cooley-Tukey Horizontal stage pass, see \autoref{sec:ct-impl}},label={lst:glsl-ct-horizontal}]
 #version 440
...
% TODO: Add Cooley-Tukey full pass Horizontal
% TODO: Add Cooley-Tukey full pass Vertical
% TODO: Add Radix-2 Stockham full pass Horizontal
% TODO: Add Radix-2 Stockham full pass Vertical
% TODO: Add Radix-4 Stockham full pass Horizontal
% TODO: Add Radix-4 Stockham full pass Vertical
