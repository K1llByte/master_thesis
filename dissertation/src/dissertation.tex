...
The GPU programming model although sophisticated, at its core it provides dispatcheable thread groups, shared memories and GPU barries for synchronisation of threads, such abstractions provide data and thread level parallelism and offer to the developer more \cite{CUDAcppguide}.

% - Talk about the programming model, the concepts of a warp/Work groups and threads/local groups


\section{Fourier Transform on the GPU}
% - 2D FFT's (if not exists already)
% - Two pass approach for Horizontal and Vetical passes
% - Matrix transposition metion
